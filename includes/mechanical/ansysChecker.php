<?php
	if(isset($_SESSION["email"])){
						...
						while($row = mysqli_fetch_assoc($result)){
							$sid = $row['s_id'];
							$sql = "select * from enrollment where s_id = '$sid' and course = 'Ansys'";
							...
							if($enrollCount == 1){
								echo "<div class='row' id='buttons'>
										...
										<div style='border: 1px solid grey; border-radius: 3px; padding: 5px;'>Google Classroom Code: 3wt8xt</div>
									</div>";
								
							}else{
								echo "<div class='row' id='price'><small><strike>Rs. 2500</strike></small> Rs. 1800</div>
