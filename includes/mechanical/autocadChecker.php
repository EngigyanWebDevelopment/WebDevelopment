<?php
	if(isset($_SESSION["email"])){
						...
						while($row = mysqli_fetch_assoc($result)){
							$sid = $row['s_id'];
							$sql = "select * from enrollment where s_id = '$sid' and course = 'Autocad'";
							$enrollResult = mysqli_query($con,$sql);
							$enrollCount = mysqli_num_rows($enrollResult);
							if($enrollCount == 1){
								echo "<div class='row' id='buttons'>
										<div style='padding: 3px;'><a href='https://attendee.gotowebinar.com/register/1433371029223763970' class='card-link btn btn-primary'>Go To Webinar</a></div>
										<div style='border: 1px solid grey; border-radius: 3px; padding: 5px;'>Google Classroom Code: i1mi86</div>
									</div>";
								
							}else{
								echo "<div class='row' id='price'><small><strike>Rs. 3500</strike></small> Rs. 2000</div>
