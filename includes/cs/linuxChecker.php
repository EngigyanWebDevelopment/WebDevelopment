<?php
	if(isset($_SESSION["email"])){
						...
						while($row = mysqli_fetch_assoc($result)){
							$sid = $row['s_id'];
							$sql = "select * from enrollment where s_id = '$sid' and course = 'linux'";
							$enrollResult = mysqli_query($con,$sql);
							$enrollCount = mysqli_num_rows($enrollResult);
							if($enrollCount == 1){
								echo "<div class='row' id='buttons'>
										<div style='padding: 3px;'><a href='https://attendee.gotowebinar.com/rt/6846718671894809347' class='card-link btn btn-primary'>Go To Webinar</a></div>
										<div style='border: 1px solid grey; border-radius: 3px; padding: 5px;'>Google Classroom Code: Very Soon Available</div>
									</div>";
								
							}else{
								echo "<div class='row' id='price'><small><strike>Rs. 3500</strike></small> Rs. 2500</div>
